      ...
      try {
        ...
        Object.entries(changesByFile).forEach(([fileName, fileChanges]) => {
          ...
          try {
            ...
            execSync(`git -c user.name="hashnity" -c user.email="support@hashnity.app" add "${targetFile}"`);
            execSync(`git -c user.name="hashnity" -c user.email="support@hashnity.app" commit -m "visual_edit_${timestamp}"`);
          } catch (gitError) {
